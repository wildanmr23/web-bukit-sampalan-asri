...
function submitReservation() {
    ...
    var campingPackageValue = document
        .getElementById("campingPackage")
        .value.split(",");
    var campingCost = parseInt(campingPackageValue[0]) || 0;
    ...
    var dp = totalCost * 0.5;
    var formattedDP = formatCurrency(dp);
    var formattedTotalCost = formatCurrency(totalCost);

    var message = `Hallo, saya ${name} ingin melakukan reservasi untuk ${activities.join(
    ", "
  )} pada tanggal ${formattedDate} pukul ${time}. No. telepon : ${phone}. Biaya total : ${formattedTotalCost}. Uang muka (50%) : ${formattedDP}. Saya akan membayar uang muka dengan metode : ${paymentMethod}, tolong berikan nomer tersebut untuk saya melakukan pembayaran. Catatan tambahan : ${
